...
/** 教練端頂部列：sticky、窄螢幕收合為漢堡選單（註解：避免多行換行）。 */
export function CoachMainToolbar({ teamName, teams, currentTeamId }: Props) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-sm backdrop-blur-md supports-[backdrop-filter]:bg-white/90 dark:supports-[backdrop-filter]:bg-zinc-950/90">
        ...
        <div className="flex items-center justify-between gap-2 px-4 py-2.5 md:gap-4 md:py-3">
          ...
          >
            ...
            <Link
              key={item.href}
              href={item.href}
              className="shrink-0 text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100"
              prefetch
            >
                {item.label}
              </Link>
          ...
          <div className="flex shrink-0 items-center gap-1.5 md:hidden">
            ...
            <button
              type="button"
              className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200 shadow-sm hover:bg-zinc-50 dark:bg-zinc-950"
        ...
        {menuOpen ?
          <>
            ...
            <div
              ...
              className="absolute left-0 right-0 top-full z-50 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 py-4 shadow-lg md:hidden"
            >
              <div className="mx-auto flex max-w-5xl flex-col gap-4">
                <nav className="flex flex-col gap-1" aria-label="教練端—行動版">
                  {NAV_LINKS.map((item) => (
                    <Link
                      key={item.href}
                      href={item.href}
                      className="rounded-md px-3 py-2.5 text-base text-zinc-800 dark:text-zinc-200 hover:bg-zinc-50 dark:bg-zinc-950"
                      prefetch
                      onClick={closeMenu}
                    >
                      {item.label}
                  ...
                  <Link
                    href="/coach/events/new"
                    className="mt-2 rounded-md bg-zinc-900 px-3 py-2.5 text-center text-base font-medium text-white hover:bg-zinc-800"
                    prefetch
                    onClick={closeMenu}
                  >
                    新增事件
