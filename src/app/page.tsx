...
export default async function Home() {
  ...
  return (
    <main className="mx-auto flex min-h-full max-w-2xl flex-col gap-6 p-8 text-zinc-900">
      <h1 className="text-2xl font-semibold tracking-tight">排球隊管理（M1）</h1>
      <p className="text-sm leading-6 text-zinc-600">
        後端為 Next.js + Prisma + Postgres；教練端需{" "}
        <strong className="font-medium text-zinc-800">Clerk</strong>{" "}
        登入。若你先前的 bootstrap 使用者 Email 與 Clerk 帳號相同，首次登入會自動合併到同一筆{" "}
        <code className="rounded bg-zinc-100 px-1">User</code>。
      </p>
          ...
          <div className="flex flex-wrap items-start gap-4">
            ...
            <div className="min-w-0 flex-1 border-l border-violet-200 pl-4">
              ...
              <p className="mt-1 text-xs text-violet-900/80">
                ...
                <code className="rounded bg-white/80 px-1">active-team-id</code>）。
              </p>
            </div>
          </div>
        </section>
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-4 text-sm shadow-sm">
        <h2 className="font-medium text-zinc-800">教練端</h2>
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-4 text-sm shadow-sm">
        <h2 className="font-medium text-zinc-800">球員端</h2>
        <p className="mt-2 text-zinc-600">
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-4 text-sm shadow-sm">
        <h2 className="font-medium text-zinc-800">訓練計畫 API</h2>
        <ul className="mt-2 list-disc space-y-1 pl-5 font-mono text-xs text-zinc-600">
          <li>GET/POST/PUT/DELETE /api/events/[id]/training-plan</li>
          <li>POST /api/events/[id]/training-plan/ai（需 DEEPSEEK_API_KEY）</li>
        </ul>
      </section>
      <section className="rounded-lg border border-zinc-200 bg-white p-4 text-sm shadow-sm">
        <h2 className="font-medium text-zinc-800">快速步驟</h2>
        <ol className="mt-2 list-decimal space-y-2 pl-5 text-zinc-600">
          <li>
            設定 <code className="rounded bg-zinc-100 px-1">.env</code>：{" "}
            <code className="rounded bg-zinc-100 px-1">DATABASE_URL</code>、Clerk 金鑰（見{" "}
            <code className="rounded bg-zinc-100 px-1">.env.example</code>）
          </li>
          <li>
            <code className="rounded bg-zinc-100 px-1">npm run db:setup</code> 套用資料表
          </li>
          <li>
            開發可設 <code className="rounded bg-zinc-100 px-1">ALLOW_BOOTSTRAP=1</code>，POST{" "}
            <code className="rounded bg-zinc-100 px-1">/api/bootstrap</code>；請用與 Clerk 相同 Email 註冊以便對應隊員資料
          </li>
          <li className="text-zinc-500">
            （選）API 腳本除錯：<code className="rounded bg-zinc-100 px-1">ALLOW_DEBUG_AUTH=true</code> 時仍可用 header／debug cookie
          </li>
        </ol>
      </section>
      <p className="text-sm text-zinc-600">
        規格：<code className="rounded bg-zinc-100 px-1">docs/volleyball-team-manager/</code>
      </p>
    </main>
  );
}
