                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                      場次：{formatDateTimeZh(fb.event.startsAt, { dateStyle: "short", timeStyle: "short" })}
                    </p>
                    ...
                    <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                      送出時間：{formatDateTimeZh(fb.submittedAt)}
                    </p>
                  </div>
                  <div className="flex shrink-0 flex-col items-start gap-1 sm:items-end">
                    {editable ?
                      <span className="rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-medium text-emerald-900">
                        可編輯至 {formatDateTimeZh(editDeadline, { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                      </span>
                    : (
