    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="rounded border border-zinc-300 bg-white px-3 py-1.5 text-xs hover:bg-zinc-50"
        >
          全部勾選實到
        </button>
        <button
      ...
      <div className="overflow-x-auto rounded-lg border border-zinc-200">
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-zinc-200 bg-zinc-50">
            <tr>
              <th className="px-3 py-2 font-medium text-zinc-600">姓名</th>
              <th className="px-3 py-2 font-medium text-zinc-600">RSVP</th>
              <th className="px-3 py-2 font-medium text-zinc-600">原因</th>
              <th className="px-3 py-2 font-medium text-zinc-600">實到</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100">
            {rows.map((r) => (
              <tr key={r.memberId}>
                ...
                <td className="px-3 py-2 tabular-nums text-zinc-700">{r.rsvpStatus}</td>
                <td className="max-w-[200px] truncate px-3 py-2 text-zinc-600">{r.rsvpReason ?? "—"}</td>
